...
data_path = "ts_2d_10000.mat" # <--- Change this to your own dataset
is_aws = True
...
estimator = TensorFlow(
    ...
    output_path=f's3://{bucket_name}/tf_transfer_output/'
)
job_name = f'vvv-tfTransfer-{train_subset}-{model_folder}-{data_path.split(".")[0]}-{datetime.now().strftime("%Y%m%d-%H%M%S")}'
