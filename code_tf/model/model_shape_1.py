...
class CombinedDepthMAE(tf.keras.losses.Loss):
    def __init__(self, depth_padding=0.0, alpha=0.5, name="combined_depth_mae", **kwargs):
        """
        alpha: weight for tumor MAE (0.0–1.0)
        (1-alpha): weight for global MAE
        """
    ...
    def call(self, y_true, y_pred):
        # Global MAE
        global_mae = tf.reduce_mean(tf.abs(y_true - y_pred))
        ...
        return (1.0 - self.alpha) * global_mae + self.alpha * tumor_mae
...
class ModelInit():  
    ...
    def build_model(self):
        ...
        self.model = Model(inputs=[inOP_beg, inFL_beg], outputs=[outQF, outDF, outShape])

        self.model.compile(
        loss={
                'outQF': 'mae',
                # 'outDF': CombinedDepthMAE(depth_padding=self.params['depth_padding'], alpha=0.5),
                'outDF': 'mae',
                'outShape': focalBCEDice(alpha=0.25, gamma=2.0, dice_weight=0.5)
        },
        loss_weights={'outQF': 1.0, 'outDF': 1.0, 'outShape': 1.0},
        ...
        metrics={
                ...
                'outDF': [
                metrics.MeanAbsoluteError(name='mae_df'),
